...
public class FitChart extends View {
    ...
    if (!isInEditMode()) {
      if (getBackground() == null) {
        setBackgroundColor(getContext().getResources().getColor(
            com.txusballesteros.widgets.R.color.default_back_color));
      }
    }
  }
  ...
  private void readAttributes(AttributeSet attrs) {
    ...
    valueStrokeColor = resources.getColor(com.txusballesteros.widgets.R.color.default_chart_value_color);
    backStrokeColor = resources.getColor(com.txusballesteros.widgets.R.color.default_back_stroke_color);
    strokeSize = resources.getDimension(com.txusballesteros.widgets.R.dimen.default_stroke_size);
    if (attrs != null) {
      TypedArray attributes = getContext()
          .getTheme().obtainStyledAttributes(attrs, com.txusballesteros.widgets.R.styleable.FitChart, 0, 0);
      strokeSize = attributes
          .getDimensionPixelSize(com.txusballesteros.widgets.R.styleable.FitChart_strokeSize, (int) strokeSize);
      valueStrokeColor = attributes
          .getColor(com.txusballesteros.widgets.R.styleable.FitChart_valueStrokeColor, valueStrokeColor);
      backStrokeColor = attributes
          .getColor(com.txusballesteros.widgets.R.styleable.FitChart_backStrokeColor, backStrokeColor);
      int attrAnimationMode = attributes.getInteger(com.txusballesteros.widgets.R.styleable.FitChart_animationMode, ANIMATION_MODE_LINEAR);
